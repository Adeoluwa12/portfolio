...
const components: Components = {
  h1: ({ children }) => (
    <h1 className="font-display text-2xl font-semibold text-text mt-10 mb-4 leading-snug">
      {children}
    </h1>
  ),
  h2: ({ children }) => (
    <h2 className="font-display text-xl font-semibold text-text mt-8 mb-3 leading-snug">
      {children}
    </h2>
  ),
  h3: ({ children }) => (
    <h3 className="font-display text-lg font-semibold text-text mt-6 mb-2 leading-snug">
      {children}
    </h3>
  ),
  h4: ({ children }) => (
  ...
  ul: ({ children }) => (
    <ul className="list-disc list-outside pl-5 mb-5 space-y-1.5 text-[0.95rem] text-text">
      {children}
    </ul>
  ),
  ol: ({ children }) => (
    <ol className="list-decimal list-outside pl-5 mb-5 space-y-1.5 text-[0.95rem] text-text">
      {children}
    </ol>
  ),
  ...
  img: ({ src, alt }) => (
    <img
      src={src}
      alt={alt ?? ""}
      className="w-full rounded-xl border border-hairline my-6 object-cover"
    />
  ),
  table: ({ children }) => (
    <div className="overflow-x-auto mb-5">
      <table className="w-full text-sm border-collapse border border-hairline rounded-lg overflow-hidden">
        {children}
      </table>
    </div>
  ),
  thead: ({ children }) => (
    <thead className="bg-surfaceAlt text-textDim font-mono text-xs uppercase tracking-wide">
      {children}
    </thead>
  ),
  ...
  th: ({ children }) => (
    <th className="px-4 py-2.5 text-left border-b border-hairline">{children}</th>
  ),
